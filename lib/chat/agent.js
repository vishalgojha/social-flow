...
class AutonomousAgent {
  ...
  async heuristicDecision(userInput) {
    ...
    if (!intent || !intent.action || intent.action === 'unknown_input' || !this.isSupportedTool(intent.action)) {
      return {
        message: 'I could not map that to a safe executable action yet. Tell me in a direct form like "show my Facebook pages" or "check auth status".',
        actions: [],
        needsInput: true,
        suggestions: [
          'Try: "show my Facebook pages"',
